...
export function AgentPricing({ slug }: { slug: string }) {
  ...
  /* Цену показывает только то, что можно купить.
   
     Двух условий здесь не одно, а два, и второе важнее. Вилки могут быть
     посчитаны заранее — у агента, который ещё пишется, цена вполне может
     быть известна. Но показать её рядом с «în curând» значит поставить на
     одну страницу «этого пока нет» и «платите столько». Ровно это уже
     случилось с конфигуратором, только наоборот, и вылезло на витрину.
   
     Поэтому цена ждёт статуса `shipped`. Как только агент выйдет, блок
     появится сам — править здесь ничего не придётся. */
  if (!product?.tiers || product.status !== 'shipped') return null;
