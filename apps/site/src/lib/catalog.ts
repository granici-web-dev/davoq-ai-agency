...
const AGENT_BASE: Array<{ slug: AgentSlug; full?: boolean }> = [
  ...
  /* Двое последних смотрят не туда, куда первые шестеро. Аналитик — внутрь
     компании, outreach — наружу, но к тем, кто ещё не клиент. Поэтому они
     идут в конце везде, где список по порядку: человек приходит на сайт за
     агентом, который отвечает его покупателям, и только потом узнаёт, что
     есть ещё двое — для него самого. */
  { slug: 'data-analyst', full: true },
  /* Без `full`: полной страницы у него нет. Писать её сейчас значило бы
     подробно расписывать то, чего ещё не существует ни строкой кода. */
  { slug: 'outreach' },
];
