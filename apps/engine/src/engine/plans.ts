...
export interface PlanFeatures {
  ...
  voice: boolean;
  /**
   * Холодные email-кампании: ищет клиентов и вербует партнёров.
   * НЕ РЕАЛИЗОВАНО В ЭТОМ ДВИЖКЕ.
   *
   * Как и аналитик, это отдельная система со своим кодом и своей базой.
   * Ключ заведён раньше кода намеренно: манифест продукта обязан сослаться
   * на существующую возможность, иначе сборка контракта не пройдёт, — и
   * агент попадает в каталог со статусом `planned` и честной пометкой,
   * вместо того чтобы жить обещанием на сайте и ничем в коде.
   *
   * `false` во всех тарифах, включая enterprise, и это не забывчивость:
   * поставить `true` значило бы пообещать доступ, которого движок не откроет.
   */
  outreach: boolean;
...
const BASE_FEATURES: PlanFeatures = {
  // Отдельная система: тарифом движка не выдаётся ни на одной ступени.
  outreach: false,
...
export const PLANS: Record<PlanId, Plan> = {
  ...
  enterprise: {
    ...
    features: {
      ...
      // Два `false` в тарифе, где всё остальное `true`, — и это не
      // забывчивость. Аналитик и outreach не запускаются этим движком,
      // поэтому движок их и не может выдать: это отдельные продукты со
      // своим кодом и своей подпиской. Поставить здесь `true` значило бы
