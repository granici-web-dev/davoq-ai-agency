...
/**
 * Запасной порог косинусной близости. Ниже него контекст не подставляется вовсе:
 * подсунуть модели нерелевантный кусок хуже, чем не подсунуть ничего — она
 * начнёт его пересказывать.
 *
 * Запасной, потому что рабочий приходит из вертикали и переопределяется
 * клиентом: значение зависит от языка и характера корпуса, то есть это свойство
 * ниши, а не процесса. Здесь остаётся то, чем пользуются вызовы без контекста
 * ниши — CLI-поиск и диагностика.
 *
 * Подбирается командой `admin.ts rank` на реальных вопросах клиента: она
 * показывает, где проходит граница между попаданием и мусором. У dev-заглушки
 * абсолютные значения не имеют смысла вовсе (она лексическая), сравнивать
 * по ним порог бесполезно.
 */
...
/** Пороги, переданные вызывающим. Не заданы — берутся запасные, см. выше. */
